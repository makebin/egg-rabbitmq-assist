...
module.exports = app => {
  ...
  if (app.config.rabbitmq.client && app.config.rabbitmq.clients) {
    return app.coreLogger.info('[egg-rabbitmq-assist] egg:singleton RabbitMQ can not set options.client and options.clients both');
  }
...
async function createRabbitMQConnection(config, app) {
  assert(config.url || config.rabbitmq, "[egg-rabbitmq-assist] url and RabbitMQ config params can't be empty at the same time on config");

  const rabbitConnectOptions = config.url || getConfigParseOptions(config.rabbitmq);
  app.coreLogger.info(`[egg-rabbitmq-assist] last connection config params on ${JSON.stringify(rabbitConnectOptions)}`);

  try {
    const client = await amqplib.connect(rabbitConnectOptions, config.socketOptions);

    app.coreLogger.info('[egg-rabbitmq-assist] use egg-rabbitmq-assist plugin to connect RabbitMQ successfully');

    return client;
  } catch (e) {
    app.coreLogger.info('[egg-rabbitmq-assist] using egg-rabbitmq-assist plugin to connect RabbitMQ append to an issue');
  }
